...
def refractive_index (lambda_min,lambda_max,Temp): #in microns and °K        CALCULATES THE AVERAGE REFRACTIVE INDEX OVER WAVELENGTH RANGE
    ...
    refraction ['Ml']=(2*math.pi*h*c**2/((refraction['wl'])*10**-6)**5)/(math.e**((0.0144)/(refraction['wl']*10**-6)/(Temp+273.15)-1))     #spektrale spez.Ausstahrlung

    refraction=refraction[(refraction['wl'] >=lambda_min) & (refraction['wl'] <= lambda_max)]   #filter Vaukles in wavelength
...
def transmissivity (RH,Temp,distance,lambda_min,lambda_max,PLWater):
    ...
    esw = (10**(22.5518+(-2937.4/(Temp)))*(Temp)**(-4.9283))*10    # in mbar
    H1 = 100/Rv/(Temp)*esw
    Vh20_RH100 =  H1*1000  #mm/km
    h_RH=RH*Vh20_RH100*distance*10**-3

    #load Passmann Larmore Tables (load this file in original file to be faster)
